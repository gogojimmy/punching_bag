  ...
  task :combine, [:by_day_after, :by_month_after, :by_year_after] => [:environment] do |t, args|
    ...
    punchable_types.each do |punchable_type|
      ...
      punchable_ids.each do |punchable_id|
        ...
        # by_year
        punchable.punches.before_by_p(args[:by_year_after].years.ago).each do |punch|
          punch.combine_by_year
        end

        # by_month
        punchable.punches.before_by_p(args[:by_month_after].months.ago).each do |punch|
          punch.combine_by_month
        end

        # by_day
        punchable.punches.before_by_p(args[:by_day_after].days.ago).each do |punch|
